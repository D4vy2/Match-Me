...
router.get('/home', function(req, res) {
    if (req.session.userId == undefined) {
        ...
        res.redirect('/users/myphotos');
    }
});

router.get('/myprofil', function(req, res) {
	if (req.session.userId != undefined) {
		Image.checkIfMainExist(req.session.userId, function(mainPix) {
			if (mainPix.length == 0) {
				...
				User.getById(req.session.userId, function(user) {
					...
					User.getMainPic(req.session.userId, user, function(err) {
						Notification.get(req.session.userId, function(notifs) {
							res.render('pages/myprofil', { error: error, data:{user}, tags:{rows}, userTags: userTags, currentUser: user.login, notification: notifs});
						})
					})
				})
			})
...
router.post('/myprofil', checkProfil, function(req, res) {
	if (res.locals.error != undefined) {
		req.session.error = res.locals.error;
		console.log('*****************   STOP !!!!! = ' + res.locals.error)
	...
	// req.session.currentUser = profil; // SAVE USER OBJECT IN SESSION
	// req.session.currentUser.tags = req.body.tags;
...
router.get('/myphotos', function(req, res) {
	...
	delete req.session.error;
	Notification.get(req.session.userId, function(notif) {
		Image.getAll(req.session.userId, function(rows) {
			if (rows.length > 0) {
				res.locals.images = rows;
			}
			res.render('pages/myphotos', {error: err, currentUser: req.session.currentUser.login, notification: notif});
		})
	})
})

router.post('/myphotos', function(req, res) {
    User.getById(req.session.userId, function(user) {
      ...
      if (file == undefined || (file.mimetype != 'image/jpeg' && file.mimetype != 'image/png')) {
				...
				let path = 'public/gallery/' + user.login + '/' + (user.login + '_' + timestamp);
				let imgName = '/static/gallery/' + user.login + '/' + (user.login + '_' + timestamp);
...
router.post('/myphotos/remove', function(req, res) {
	if (req.body.hidden == "") {
		res.redirect('/users/myphotos');
		return;
	}
...
router.post('/myphotos/setmain', function(req, res) {
	if (req.body.hidden == "") {
		res.redirect('/users/myphotos');
		return;
	}
...
router.get('/activity', function(req, res) {
	if (req.session.userId == undefined || req.session.userId == "") {
		...
		Notification.get(req.session.userId, function(rows) {
			Notification.getHistory(req.session.userId, function(activity) {
				res.render('pages/activity', {currentUser: req.session.currentUser.login, notification: rows, activity: activity});
			})
		});
	}
})
